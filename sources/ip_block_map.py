...
def template_read(path_template: pathlib.Path) -> str:
    ...
    if "</head>" not in text_template:
        raise ValueError(f"template has no </head> to hold the map styles: {path_template}")
...
def page_write(
    ...
    The map styles go into the document head. Putting stylesheet text in the
    content would render it as visible text, which is a mistake this code made
    once.
    """
