...
def page_write(
    path_page: pathlib.Path, text_template: str, text_title: str, text_content: str
) -> None:
    """Write one page from the template."""
    path_page.parent.mkdir(parents=True, exist_ok=True)
    text_page = text_template.replace("__TITLE__", html.escape(text_title))
    text_page = text_page.replace(MARKER_CONTENT, STYLE_PAGE + text_content)
    with open(path_page, "w", encoding="utf-8") as file_page:
        file_page.write("<!-- generated by sources/ip_block_map.py; do not edit manually -->\n")
...
def block_list_read(path_block: pathlib.Path) -> list[dict]:
    ...
    for document_block in list_block:
        ...
        document_block["_end"] = probe_parse(document_block["derived"]["address_end"])
...
def row_build(
    ...
    document_derived = document_block["derived"]
    ...
    text_range = (
        document_derived.get("prefix")
        or f"{document_rir['start']} - {document_derived['address_end']}"
    )
...
def block_page_build(
    ...
    list_html = [
        ...
        f"{html.escape(document_derived['address_end'])}</td></tr>",
        f"<tr><th>Prefix</th><td>"
        f"{html.escape(str(document_derived.get('prefix') or 'not a CIDR block'))}</td></tr>",
        ...
        f"<tr><th>Opaque id</th><td>{html.escape(document_derived.get('opaque_id') or '—')}</td></tr>",
    ...
    if list_anomaly:
        ...
        for text_address in list_anomaly:
            ...
            list_html.append(
                f"<tr><td>{html.escape(text_address)}</td>"
